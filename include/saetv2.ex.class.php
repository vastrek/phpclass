...
class SaeTClientV2
{
	 ...
	 *
	 * 返回用户发布的第一条微博
	 * <br />对应API：{@link http://open.weibo.com/wiki/2/statuses/user_timeline/ids}
	 * 
	 * @access public
	 * @param mixed $uid 指定用户UID或微博昵称
	 * @return array
	 */
	function statuses_get_first( $uid = NULL)
	{
		$params = array();
		$params['uid']=$uid;
		$params['count']=1;
    $res=$this->oauth->get( 'statuses/user_timeline/ids', $params );
    $firstId=0;
    if($res['total_number'] > 0){
    	  $params['count']=5;
    	  $params['page']=ceil($res['total_number']/5);
    		$res=$this->oauth->get( 'statuses/user_timeline/ids', $params );
    		//获取最后一个元素
    		$firstId=array_pop($res['statuses']);
    }
    if($firstId==0){
    	return false;	
    }
		return $this->show_status($firstId);
	}
	 ...
	 * @return array
	 */
	function statuses_get_all_ids($uid = NULL,$allIds){
		$count=200;
		$params = array();
		$params['uid']=$uid;
		$params['count']=$count;
	 ...
	 * @return array
	 */
	function statuses_get_max_reposts($allIds){
		if(count($allIds)==0){
			return 0;	
		}
		$count=100;
		...
		$maxReposts=0;
		while(true){
			$ids=array_slice($allIds, $offset,$count);
			if(count($ids)==0){
				return $maxId;	
			}
			$offset+=$count;
			$params['ids']=join(",",$ids);
	    $res=$this->oauth->get( 'statuses/count', $params );
	    foreach($res as $row){
	    		if($maxReposts < $row['reposts']){
	    				$maxId=$row['id'];
	    				$maxReposts = $row['reposts'];
	    		}
	    }
  	}
  	return $maxId;
...
/**
	 ...
	 * @return array
	 */
	function friends( $uid, $cursor = 0, $count = 200 )
	{
		$params = array();
		$this->id_format($uid);
		$params['uid'] = $uid;
...
/**
	 ...
	 * @return array
	 */
	function search_topics( $q,  $count = 10 ,$page = 1)
	{
		$params = array();
		$params['q'] = $q;
		$params['count'] = $count;
...
/**
	 * 退出
	 *
	 * 对应API：{@link http://open.weibo.com/wiki/2/account/end_session}
	 * 
	 * @access public
	 * @return array
	 */
	 ...
	 * ip to geo
	 *
	 * 对应API：{@link http://open.weibo.com/wiki/2/location/geo/ip_to_geo suggestions/favorites/hot}
	 *
	 * @param int $count 每页返回结果数，默认20。选填
	 * @param int $page 返回页码，默认1。选填
	 * @return array
	...
	function ip_to_geo( $ip )
	{
		$params = array();
		$params['ip'] = $ip;
		return $this->oauth->get( 'location/geo/ip_to_geo', $params);
	}
}
